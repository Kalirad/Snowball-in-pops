...
class Population(object):
    ...
    def __init__(self, seq, pop_size, mut_rate=1e-4, rec_rate=0., alpha=12, t_burnin=200):
        ...
        self.population = np.array([RNASeq.convertor(seq) for i in range(pop_size)])
    ...
    def introgression_assay(self, other):
...
class TwoPops(object):
    ...
    def update_history(self):
        ...
        # Introgression 2 -> 1
        temp = self.pop1.introgression_assay(self.pop2)
        ...
        # Introgression 1 -> 2
        temp = self.pop2.introgression_assay(self.pop1)
    ...
    def evolve(self, gen, step=100, verbose=False):
        ...
        for i in np.arange(1, gen + 1, 1):
            ...
            if not i%step:
                self.update_history()
                if verbose:
                    print i, self.pop1.N, self.pop2.N

    """
    def evolve_till_diverge(self, d_threshold=40, step=100):
        self.init_history()
        d = 0
        self.update_history()
        gen = 0
        while d < d_threshold:
            ...
            if self.mig_rate:
                self.migrate()
            d = RNASeq.get_hamdist(self.pop1.wt_seq, self.pop2.wt_seq)
            gen += 1
        print gen, d
    """
