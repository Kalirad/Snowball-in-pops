...
class Population(object):
    ...
    def __init__(self, seq, pop_size, mut_rate=1e-4, rec_rate=0., alpha=12, t_burnin=200):
        ...
        assert type(rec_rate) == float and 0 <= rec_rate <= 1.
    ...
    def recombine_pop(self, other=False, rec_max=False):
        ...
        Keyword Arguments:
            ...
            rec_max {bool} -- Generates all the possible recombinants between  
            the two samples (default: {False}).
...
class TwoPops(object):
    ...
    def update_history(self, sample_n=10):
        ...
        double_inv = []
        triple_inv = []
        for i,j in zip(pop2_sample, pop1_sample):
            temp = self.pop2.introgression_assay(i,j)
            introg_results.append(temp)
            p1_inv.append(temp['p1'])
            p2_inv.append(temp['p2'])
            p3_inv.append(temp['p3'])
            single_inv.append(len(temp['single']))
            double_inv.append(len(temp['double']))
            triple_inv.append(len(temp['triple']))
        self.introg_results_inv.append(introg_results)
        self.p1_inv.append(np.mean(np.array(p1_inv)))
        self.p2_inv.append(np.mean(np.array(p2_inv)))
        self.p3_inv.append(np.mean(np.array(p3_inv)))
        self.single_inv.append(np.mean(np.array(single_inv)))
        self.double_inv.append(np.mean(np.array(double_inv)))
        self.triple_inv.append(np.mean(np.array(triple_inv)))
        """
        #self.viable_sin_pair_inv.append(temp['viable_sin_pair'])
        # Within pop introgression in pop 1 to find segregating -i.e., "seg"- DMIs.
        ...
        self.single_seg.append(np.mean(np.array(single_seg)))
        self.double_seg.append(np.mean(np.array(double_seg)))
        self.triple_seg.append(np.mean(np.array(triple_seg)))
        # RI
        temp = self.get_RI()
        ...
        pop1_rec_load = self.pop1.rec_load
        pop2_rec_load = self.pop2.rec_load
        self.rec_load.append(np.mean([pop1_rec_load, pop2_rec_load]))
        # Check pop size
    ...
    def migrate(self):
        ...
        for i in range(len(migrants)):
            if migrants[i]:
                self.pop1.population[i] = deepcopy(pop1_temp[i])
                self.pop2.population[i] = deepcopy(pop2_temp[i])
    ...
    def evolve(self, gen, step=100, verbose=False):
        ...
        Keyword Arguments:
            step {int} -- The intervals at which the relevant statistics are to be saved (default: {500})
        ...
        for i in np.arange(1, gen + 1, 1):
            ...
            if not i%step:
                self.update_history()
                if verbose:
                    print i, len(self.pop1.population), len(self.pop2.population)
